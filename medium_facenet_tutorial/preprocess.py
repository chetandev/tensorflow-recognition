...
def preprocess_image(input_path, output_path, crop_dim):
    ...
    if image is not None:
        ...
        for im in image :
            global person_id
            image_output_dir = os.path.join(output_path, os.path.basename(os.path.dirname(input_path)))#+ "_person_" +str(person_id) )
            if not os.path.exists(image_output_dir):
                os.makedirs(image_output_dir)
            # out_path = os.path.join(image_output_dir, os.path.basename(input_path))
            out_path = os.path.join(image_output_dir, str(i) + "_" + os.path.basename(input_path))
...
def _align_image(image, crop_dim):
    bbarray = align_dlib.getAllFaceBoundingBoxes(image)
    # print (len(bbarray))
    alignedarray = []
    aligned2array = []
    for bb in bbarray :
        alignedarray.append(align_dlib.align(crop_dim, image, bb, landmarkIndices=AlignDlib.INNER_EYES_AND_BOTTOM_LIP))
    if len(alignedarray) > 0:
        for aligned in alignedarray:
            aligned = cv2.cvtColor(aligned, cv2.COLOR_BGR2RGB)
            aligned2array.append(aligned)
    return aligned2array


if __name__ == '__main__':
    logging.basicConfig(level=logging.INFO)
    parser = argparse.ArgumentParser(add_help=True)
    parser.add_argument('--input-dir', type=str, action='store', default='../data2', dest='input_dir')
    parser.add_argument('--output-dir', type=str, action='store', default='../output2', dest='output_dir')
